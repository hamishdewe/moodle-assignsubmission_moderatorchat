 ...
 * @return bool
 */
function assignsubmission_moderatorchat_comment_validate(stdClass $options) {
    global $USER, $CFG, $DB;

    if ($options->commentarea != 'submission_moderatorchat' &&
            $options->commentarea != 'submission_moderatorchat_upgrade') {
 ...
 * @return array
 */
function assignsubmission_moderatorchat_comment_permissions(stdClass $options) {
    global $USER, $CFG, $DB;
 ...
 */
function assignsubmission_moderatorchat_comment_display($comments, $options) {
    global $CFG, $DB, $USER;
    ...
    if ($assignment->is_blind_marking() && !empty($comments)) {
        ...
        foreach ($comments as $comment) {
            ...
            if (has_capability('mod/assign:viewblinddetails', $context) && $USER->id != $comment->userid) {
                ...
                $comment->fullname = get_string('blindmarkingviewfullname', 'assignsubmission_moderatorchat', $a);
                ...
                if (empty($usermappings[$comment->userid])) {
                    ...
                    $commenter->firstname = get_string('blindmarkingname', 'assignsubmission_moderatorchat', $anonid);
                    $commenter->lastname = '';
 ...
 */
function assignsubmission_moderatorchat_comment_add(stdClass $comment, stdClass $param) {

    global $DB;
    if ($comment->commentarea == 'submission_moderatorchat_upgrade') {
        $submissionid = $comment->itemid;
        $submission = $DB->get_record('assign_submission', array('id' => $submissionid));

        $comment->userid = $submission->userid;
        $comment->commentarea = 'submission_moderatorchat';
    }
}
